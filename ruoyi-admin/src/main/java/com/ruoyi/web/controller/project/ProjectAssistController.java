...
import com.ruoyi.project.service.ProjectFeaturesService;
...
public class ProjectAssistController extends BaseController {
    ...
    public AjaxResult removeVersionTask(@RequestBody Map<String,Object> mmap) throws Exception {
        ...
        Map taskMap = taskProjectService.getTaskMapByCode(taskCode);
    ...
    public AjaxResult getVersionTask(@RequestBody Map<String,Object> mmap){
        ...
        List<Map>  taskList = taskProjectService.getTaskListByVersionAndDelete(param);
        ...
        if(!CommUtils.isEmpty(taskCodeList)){
            ...
            if(jsonArray != null && jsonArray.size() != 0){
                ...
                for (Object obj : jsonArray) {
                    Map taskMap = taskProjectService.getTaskMapByCode(String.valueOf(obj));
                    if(MapUtils.isEmpty(taskMap)){
                        return AjaxResult.warn("该任务已被失效");
                    }
    ...
    public AjaxResult getProjectFile(@RequestBody Map<String,Object> mmap){
        ...
        page_=fileService.lambdaQuery().eq(com.ruoyi.task.domain.File::getProjectCode,projectCode).eq(com.ruoyi.task.domain.File::getDeleted,deleted).page(page_);
