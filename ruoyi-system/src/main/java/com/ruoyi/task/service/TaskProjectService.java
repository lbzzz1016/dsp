...
import com.ruoyi.task.mapper.TaskToTagMapper;
...
@Slf4j
@Service
public class TaskProjectService extends ServiceImpl<TaskMapper, Task> {
    ...
    public List<Map> tasks(Map param){
        ...
        if(!CollectionUtils.isEmpty(mapList)){
            ...
            for(Map map : mapList){
                ...
                map = buildTaskMap(map, LoginHelper.getLoginUser().getCode());
                resultList.add(map);
            }
        }
        return resultList;
    ...
    public void batchAssignTask(String taskCodes,String executorCode){
        JSONArray jsonArray = JSONUtil.parseArray(taskCodes);
...
//    private MemberService memberService;
    ...
    public AjaxResult createTask(Task task, String pcode){
        ...
        taskHook(LoginHelper.getLoginUser().getCode(),task.getCode(),"create","",0,
                "","","",null,null);
        if(StringUtils.isNotEmpty(pcode)){
            taskHook(LoginHelper.getLoginUser().getCode(),pcode,"createChild","",0,
                    "","","",new HashMap(){{
    ...
    public void recovery(String taskCode){
        ...
        taskHook(LoginHelper.getLoginUser().getCode(),taskCode,"recovery","",0,
                "","","",null,null);
    ...
    @Autowired
    CollectionTeamService collectionTeamService;
    ...
    public void star(Map taskMap,String memberCode,Integer starData){
        ...
        collectionTeamService.starTask(code,memberCode,star);
    }
...
//                    webSocketServer.sendInfo(build, taskMemberCode);
    ...
    public void createComment(String taskCode,String comment,String mentions){
        ...
        if(StringUtils.isNotEmpty(mentions)){
            JSONArray jsonArray = JSONUtil.parseArray(mentions);
            for (Object obj : jsonArray) {
                mentionList.add(String.valueOf(obj));
            }
        }
        taskHook(LoginHelper.getLoginUser().getCode(),taskCode,"comment","",1,
                "",comment,"",new HashMap(){{
...
//    			.parallelStream().collect(Collectors.toMap(Member::getName, Member::getCode));
        ...
        String formatDate = LocalDateTime.now().format(DateTimeFormatter.ofPattern(DateUtils.YYYY_MM_DD_HH_MM_SS));
